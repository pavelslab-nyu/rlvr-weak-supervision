...
def process_validation_metrics(
    ...
    # Compute per-sample-index metrics (simulating independent runs)
    # For each sample index (0, 1, 2, ...), compute accuracy across all prompts
    # This gives [acc_run0, acc_run1, ...] which simulates independent evaluation runs
    for data_source, prompt2var2vals in data_src2prompt2var2vals.items():
        # Check if we have prompts and rewards
        if not prompt2var2vals:
            continue
        
        first_prompt_vals = next(iter(prompt2var2vals.values()))
        
        # Process reward and acc variables
        for var_name in ["reward", "acc"]:
            if var_name not in first_prompt_vals:
                continue
            
            n_samples = len(first_prompt_vals[var_name])
            if n_samples <= 1:
                continue  # No point computing for single sample
            
            # Collect rewards/scores by sample index
            sample_idx_scores = [[] for _ in range(n_samples)]
            for prompt, var2vals in prompt2var2vals.items():
                vals = var2vals.get(var_name, [])
                for idx, score in enumerate(vals):
                    if idx < n_samples:  # Safety check
                        sample_idx_scores[idx].append(score)
            
            # Compute per-"run" accuracy (mean across all prompts for each sample index)
            run_accuracies = [np.mean(scores) if scores else 0.0 for scores in sample_idx_scores]
            
            # Add individual run accuracies
            for i, acc in enumerate(run_accuracies):
                data_src2var2metric2val[data_source][var_name][f"indep_run/run_{i}"] = acc
            
            # Add mean, std, and best (max) across runs
            data_src2var2metric2val[data_source][var_name]["indep_run/mean"] = np.mean(run_accuracies)
            data_src2var2metric2val[data_source][var_name]["indep_run/std"] = np.std(run_accuracies)
            data_src2var2metric2val[data_source][var_name]["indep_run/passk"] = np.max(run_accuracies)
